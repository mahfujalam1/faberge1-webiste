...
import { useGetTermsAndConditionsQuery } from "@/redux/api/publicApi";

const TermsAndConditionsPage = () => {
  const { data, isLoading } = useGetTermsAndConditionsQuery(undefined);
  const content = data?.data?.termsAndConditions || data?.termsAndConditions || "";

  return (
    <div className="min-h-screen bg-gray-50">
      {/* <DynamicBanner title="Terms & Conditions" /> */}
        ...
        {isLoading ? (
          ...
          <div className="bg-white p-8 md:p-12 rounded-2xl shadow-sm border border-gray-100">
           <div 
            ...
            />
            {!content && !isLoading && (
              <p className="text-center text-gray-500">No terms and conditions found.</p>
            )}
          </div>
        )}
      </div>
